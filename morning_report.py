...
def is_kr_market_open():
    try:
        ...
        # 현재 시간을 KST로 변환 후 날짜 추출
        today_kst = datetime.now(KST).strftime("%Y-%m-%d")
        return krx.is_session(today_kst)
    except:
...
def get_market_status():
    try:
        ...
        return {"date": datetime.now(KST).strftime("%Y-%m-%d"), "status": "확인불가", "usd_krw": 0, "vix": 0, "rate_chg_pct": 0, "total_weight": "데이터 확인 불가"}
...
if __name__ == "__main__":
    ...
    if not market_open:
        report_header = f"🚩 <b>오늘은 국장 휴장일입니다. ({m['date']})</b>\n"
    ...
    try:
        ...
        if market_open:
            ...
            print(f"✨ {m['date']} 리포트 발송 및 데이터 기록 완료!")
        else:
            print("✨ 휴장일 리포트 발송 완료")
